...
var img_src = './map2.png'; // var image = L.imageOverlay('./map2.png', bounds).addTo(mymap);

var map_width = getImgSize(img_src)[0];
var map_height = getImgSize(img_src)[1];
console.log(map_width, map_height); // mymap.fitBounds(bounds)
...
console.log("image bounds: ", bounds); // add the image overlay, 
// so that it covers the entire map

var overlay = L.imageOverlay(img_src, bounds).addTo(map); // tell leaflet that the map is exactly as big as the image
...
function onMapClick(e) {
  ...
  var clientClick = map.project(e.latlng); //Grab the original overlay

  var overlayImage = overlay._image; //Calculate the current image ratio from the original
  ...
  console.log("overlay natural height: ", overlayImage.naturalHeight); //scale the x,y coordinate of the click based on tile scaling of the overlay layer

  var x = clientClick.x / xR;
  var y = overlayImage.naturalHeight - clientClick.y / yR;
  console.log(x, y);
  var pointXY = pixel2MapLocation(x, y); // popup
  //     .setLatLng(e.latlng)
